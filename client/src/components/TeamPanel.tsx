    ...
    <div className="flex flex-col h-full gap-4">
      ...
      <GlassPane
        ...
        headerAction={onInvite && (
          <button
            onClick={onInvite}
            className="text-[11px] bg-indigo-50 text-indigo-600 px-2 py-1 rounded hover:bg-indigo-100 transition font-bold uppercase tracking-wider"
          >
            + Invite
          </button>
        )}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <p className={`text-sm font-bold ${currentUser.id === user.id ? 'text-indigo-900' : 'text-slate-700'}`}>
                    {user.name} {currentUser.id === user.id && <span className="text-[11px] text-indigo-500 font-normal">(You)</span>}
                  </p>
          ...
          {activities.map((activity) => (
            <div key={activity.id} className="relative pl-4 border-l border-slate-200 pb-2 last:pb-0">
              ...
              <div className="flex flex-col">
                <span className="text-[11px] text-slate-400 font-mono">{activity.timestamp}</span>
                <p className="text-xs text-slate-700">
                  <span className="font-bold">{activity.userName}</span> {activity.action}
                </p>
                <p className="text-[11px] text-slate-500 bg-slate-50 px-2 py-1 rounded mt-1 inline-block w-fit border border-slate-100">
                  {activity.target}
                </p>
              </div>
            </div>
