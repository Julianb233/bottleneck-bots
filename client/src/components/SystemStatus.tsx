      ...
      {isOpen && (
        <div className="absolute top-full right-0 mt-2 w-72 bg-white/90 backdrop-blur-xl border border-white/60 rounded-xl shadow-2xl p-1 z-50 animate-zoom-in duration-150">
          <div className="px-3 py-2 border-b border-slate-100">
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Active Context Sources</p>
          </div>
          <div className="p-1">
            {systems.map((sys) => (
              <div key={sys.name} className="flex items-center gap-3 p-2 hover:bg-indigo-50 rounded-lg transition-colors group cursor-default">
                <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center text-[11px] font-bold text-slate-500 group-hover:bg-white group-hover:shadow-sm group-hover:text-indigo-600 transition-all">
                  {sys.icon}
                </div>
                <div className="flex-1">
                  <div className="flex justify-between items-center">
                    <p className="text-xs font-bold text-slate-700">{sys.name}</p>
                    <span className="text-[11px] text-emerald-600 font-medium bg-emerald-50 px-1.5 rounded">{sys.status}</span>
                  </div>
          ...
          <div className="bg-indigo-50/50 p-2 rounded-b-lg text-center">
             <span className="text-[11px] text-indigo-500 font-mono">System Healthy • Latency 45ms</span>
          </div>
        </div>
      )}
    </div>
