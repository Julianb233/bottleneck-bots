    ...
    <div className="flex flex-col gap-6 h-full">
      ...
      <GlassPane title="SEO Strategy Engine">
        <div className="p-4 space-y-4">
          <div>
            <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Global Page Title (H1 Context)</label>
            <input 
              type="text" 
          ...
          <div>
            <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Meta Description</label>
            <TextareaWithCount
          ...
          <div>
            <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Target Keywords</label>
          ...
          {assets.map((asset) => (
            <div key={asset.id} className="group relative bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all">
               <div className="aspect-video bg-slate-100 relative">
                  ...
                  {asset.status === 'ready' && (
                    <span className="absolute top-1 right-1 bg-emerald-500 text-white text-[11px] font-bold px-1.5 py-0.5 rounded shadow-sm">
                      SEO OPTIMIZED
                    </span>
                  )}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-black/60 backdrop-blur-sm p-1 translate-y-full group-hover:translate-y-0 transition-transform">
                     <p className="text-[11px] text-white font-mono truncate text-center">{asset.contextTag}</p>
                  </div>
               </div>
               <div className="p-2">
                  <p className="text-[11px] font-bold text-slate-700 truncate" title={asset.optimizedName}>{asset.optimizedName}</p>
                  <p className="text-[11px] text-slate-400 truncate mt-0.5" title={asset.altText}>Alt: {asset.altText}</p>
               </div>
            </div>
          ))}
        </div>
