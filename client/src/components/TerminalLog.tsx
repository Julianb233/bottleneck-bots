...
export const TerminalLog: React.FC<TerminalLogProps> = ({ logs }) => {
  ...
  return (
    <GlassPane title="Execution Log" className="h-full">
      <div className="p-2 sm:p-4 font-mono text-[11px] sm:text-xs h-full overflow-y-auto space-y-2 bg-white/20">
        ...
        {logs.map((log) => (
          <div key={log.id} className="flex items-start gap-2 group hover:bg-white/40 p-1.5 sm:p-2 rounded-lg transition-colors">
            <span className="text-slate-400 shrink-0 text-[11px] sm:text-[11px] pt-0.5">[{log.timestamp}]</span>
            <span className={`shrink-0 font-bold uppercase px-1.5 py-0.5 rounded text-[11px] sm:text-[11px] border ${getColor(log.level)}`}>
              {log.level}
            </span>
