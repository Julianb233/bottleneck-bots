  ...
  return (
    <GlassPane title="Remote Browser" className="h-full relative group">
      ...
      <div className="bg-white/80 border-b border-slate-200 p-3 flex items-center gap-3">
        ...
        {/* URL Bar */}
        <div className="bg-slate-100/80 border border-slate-200 rounded-md px-2 sm:px-4 py-1.5 text-[11px] sm:text-xs text-slate-500 font-mono flex-1 text-center truncate flex items-center justify-center gap-2 shadow-inner max-w-[200px] sm:max-w-none mx-auto">
      ...
      <div className="w-full h-full bg-slate-50 relative flex items-center justify-center overflow-hidden">
          ...
          <div className="w-full h-full flex flex-col">
            <div className={`p-2 border-b flex items-center justify-between ${isStreaming || isProcessing ? 'bg-blue-50 border-blue-200' : 'bg-emerald-50 border-emerald-200'}`}>
              <div className="flex items-center gap-2">
                ...
                {sessionId && (
                  <span className="text-[11px] text-slate-500 font-mono">ID: {sessionId.slice(0, 8)}</span>
                )}
              </div>
              <a
                href={liveViewUrl}
        ...
        {isAnalyzing && isProcessing && (
          <div className="absolute inset-0 z-10 pointer-events-none">
            ...
            <div className="absolute top-10 right-10 bg-black/80 backdrop-blur text-white text-[11px] font-mono p-2 rounded border border-white/20 shadow-xl animate-pulse">
        ...
        {isBuilding && isProcessing && (
          <div className="absolute inset-0 z-10 pointer-events-none border-4 border-emerald-500/30 border-dashed animate-pulse">
            <div className="absolute bottom-4 right-4 bg-emerald-600 text-white px-2 sm:px-3 py-1 rounded-full text-[11px] sm:text-xs font-bold shadow-lg flex items-center gap-2">
